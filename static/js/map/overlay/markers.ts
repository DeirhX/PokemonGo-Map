...
export function createPokemonMarker(item: IPokemon, pokemonSprites: any, skipNotification?: boolean): Marker {
    ...
    if (notifiedPokemon.indexOf(item.pokemon_id) > -1 || notifiedRarity.indexOf(item.pokemon_rarity) > -1) {
        if (!skipNotification) {
            ...
            const notiContent = `${ivValue ? ("IV: " + ivValue.toFixed(0) + "% and expires in") : "Expires in"} ${new Date(item.disappear_time).toLocaleTimeString("en-US", {hour12: false})}`
            sendNotification("A wild " + item.pokemon_name + " appeared!", notiContent,
